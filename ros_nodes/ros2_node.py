...
class UOAIS(Node):
    ...
    def __init__(self):
        super().__init__("uoais")

        self.mode = self.declare_parameter("~mode", "topic").value
        self.get_logger().info("Starting uoais node with {} mode".format(self.mode))
        self.rgb_topic = self.declare_parameter("~rgb", "/camera/color/image_raw").value
        self.depth_topic = self.declare_parameter("~depth", "/camera/aligned_depth_to_color/image_raw").value
        camera_info_topic = self.declare_parameter("~camera_info", "/camera/color/camera_info").value
        # UOAIS-Net
        self.det2_config_file = self.declare_parameter("~config_file", 
                            "configs/R50_rgbdconcat_mlc_occatmask_hom_concat.yaml").value
        self.confidence_threshold = self.declare_parameter("~confidence_threshold", 0.5).value
        # CG-Net foreground segmentation
        self.use_cgnet = self.declare_parameter("~use_cgnet", False).value
        self.cgnet_weight = self.declare_parameter("~cgnet_weight",
                             "foreground_segmentation/rgbd_fg.pth").value
        # RANSAC plane segmentation
        self.use_planeseg = self.declare_parameter("~use_planeseg", False).value
        self.ransac_threshold = self.declare_parameter("~ransac_threshold", 0.003).value
        self.ransac_n = self.declare_parameter("~ransac_n", 3).value
        self.ransac_iter = self.declare_parameter("~ransac_iter", 10).value
        ...
        self.listener = tf2_ros.TransformListener(self.tf_buffer, self)  # Create a tf listener
        ...
        if self.use_planeseg:
            camera_info = self.wait_for_message(camera_info_topic, CameraInfo)
            self.K = camera_info.K
            self.o3d_camera_intrinsic = None

        if self.mode == "topic":
            rgb_sub = message_filters.Subscriber(self, self.rgb_topic, Image)
            depth_sub = message_filters.Subscriber(self, self.depth_topic, Image)
            ...
            self.result_pub = self.create_publisher(UOAISResults, "/uoais/results", 10)
            self.get_logger().info("uoais results at topic: /uoais/results")

        elif self.mode == "service":
            self.srv = self.create_service(UOAISRequest, '/get_uoais_results', self.service_callback)
            self.get_logger().info("uoais results at service: /get_uoais_results")
        else:
            raise NotImplementedError
        self.vis_pub = self.create_publisher(Image, "/uoais/vis_img", 10)
        self.get_logger().info("visualization results at topic: /uoais/vis_img")
    ...
    def load_models(self):
        ...
        self.get_logger().info("Loading UOAIS-Net with config_file: {}".format(self.det2_config_file))
        self.cfg = get_cfg()
    ...
    def service_callback(self, request, response):
        ...
        rgb_msg = self.wait_for_message(self.rgb_topic, Image)
        depth_msg = self.wait_for_message(self.depth_topic, Image)
        results = self.inference(rgb_msg, depth_msg)        
    ...
    def inference(self, rgb_msg, depth_msg):
        ...
        # CG-Net
        if self.use_cgnet:
            self.get_logger().info_once("Using foreground segmentation model (CG-Net) to filter out background instances")
        ...
        # RANSAC
        if self.use_planeseg:
            self.get_logger().info_once("Using RANSAC plane segmentation to filter out background instances")
        ...
        try:
            ...
            """
            transform_stamped = self.tf_buffer.lookup_transform('base_link', depth_msg.header.frame_id, rclpy.time.Time())
            ...
            self.get_logger().error("Failed to transform point cloud: {}".format(e))
            return
