...
class UOAIS():
    ...
    def start_callback(self, msg):
        if msg.data == 0:
            rospy.loginfo("clear target and point cloud")
            self.target_pointcloud = None
            self.target_center = None
            self.target_amodal_mask = None
        elif msg.data == 1:
            rospy.loginfo("start uoais node")
            self.inferencing = True
        elif msg.data == 2:
            rospy.loginfo("end uoais node")
            self.inferencing = False
    ...
    def inference(self, rgb_msg, depth_msg):
        ...
        """
        filter out the background instances (start)
        """
        # CG-Net
        if self.use_cgnet:
        ...
        """
        filter out the background instances (end)
        """
